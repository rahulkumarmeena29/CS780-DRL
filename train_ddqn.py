...
"""
Offline trainer: Double DQN + replay buffer for OBELIX.
...
class Replay:
    def __init__(self, cap: int = 100000):
...
def main():
    ...
    ap.add_argument("--max_steps", type=int, default=600)   # faster than 1500
    ...
    ap.add_argument("--train_freq", type=int, default=8)    # speed improvement
        ...
        for _ in range(args.max_steps):
            ...
            if len(replay) >= max(args.warmup, args.batch) and steps % args.train_freq == 0:
                ...
                with torch.no_grad():
                    ...
                    next_q_online = q(s2b_t)
                    next_a = torch.argmax(next_q_online, dim=1)

                    next_q_target = tgt(s2b_t)
                    next_val = next_q_target.gather(1, next_a.unsqueeze(1)).squeeze(1)
